...
public class FormDataService {
     ...
     * - Position for the prospective date (across all same-type ledgers)
     * - Loans for the member that may start on or before that date (across all same-type ledgers)
     * - Previous underpaid amounts for the member across same-type ledgers (member-specific)
     *
    ...
    public ProspectivePayment getProspectivePaymentsForDate(
            int memberId, String ledgerType, LocalDate date) {
        ...
        try {
            ...
            BigDecimal scheduledPayment = loanService.getScheduledPaymentWithDate(memberId, ledgerType, date);
            System.out.println("DEBUG FormDataService: memberId=" + memberId + ", ledgerType=" + ledgerType + ", date=" + date);
            ...
            // 2. Find previous underpaid amount for the member across same-type ledgers (before the prospective date)
